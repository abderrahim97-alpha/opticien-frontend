...
const MontureList: React.FC = () => {
  ...
  useEffect(() => {
    // Filter montures based on search and brand
    let filtered = montures.filter((monture) => {
      const matchesSearch =
  ...
  const fetchMontures = async (token: string) => {
    try {
      ...
      const response = await api.get('/montures', {
        headers: {
      ...
      setError(err.response?.data?.message || 'Erreur lors du chargement des montures');
  ...
  const uniqueBrands = Array.from(new Set(montures.map((m) => m.brand).filter(Boolean)));
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-blue-50 via-white to-indigo-50 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-gray-600 font-medium">Chargement des montures...</p>
        </div>
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-white to-indigo-50 py-12 px-4">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="mb-8 md:mb-12 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          <div>
            <h1 className="text-2xl sm:text-3xl md:text-4xl font-bold text-gray-900 mb-1">Mes Montures</h1>
            <p className="text-sm sm:text-base text-gray-600">Gérez vos montures à vendre</p>
          </div>
          <button
            onClick={() => navigate('/montures/create')}
            className="w-full sm:w-auto bg-blue-600 text-white px-4 sm:px-6 py-2 sm:py-3 rounded-lg font-medium hover:bg-blue-700 transition flex items-center justify-center gap-2"
          >
            <svg className="w-4 sm:w-5 h-4 sm:h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 4v16m8-8H4" />
            </svg>
            <span className="text-sm sm:text-base">Ajouter une monture</span>
          </button>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4 mb-6 sm:mb-8">
          <div className="relative">
            ...
            <input
              ...
              className="w-full pl-9 sm:pl-12 pr-3 sm:pr-4 py-2 sm:py-3 text-sm sm:text-base border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent transition duration-200 outline-none"
            />
          </div>
          ...
          <select
            ...
            className="px-3 sm:px-4 py-2 sm:py-3 text-sm sm:text-base border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent transition duration-200 outline-none"
          >
        ...
        <div className="mb-4 sm:mb-6">
          <p className="text-xs sm:text-sm text-gray-600">
            Affichage de <span className="font-medium text-gray-900">{filteredMontures.length}</span> monture{filteredMontures.length !== 1 ? 's' : ''} sur{' '}
        ...
        {filteredMontures.length > 0 ? (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {filteredMontures.map((monture) => (
              <div key={monture.id} className="bg-white rounded-xl shadow-md hover:shadow-xl transition duration-300 overflow-hidden border border-gray-100">
                {/* Image Section */}
                ...
                {/* Card Body */}
                <div className="p-6">
                  <h3 className="text-lg font-bold text-gray-900 mb-2">{monture.name}</h3>

                  {monture.brand && <p className="text-sm text-gray-600 mb-2">Marque: {monture.brand}</p>}

                  {monture.description && (
                    <p className="text-sm text-gray-700 mb-4 line-clamp-2">{monture.description}</p>
                  )}
                  ...
                  <div className="flex justify-between items-center mb-4">
                    <div>
                      ...
                      {monture.stock !== undefined && (
                        <p className={`text-sm font-medium ${monture.stock > 0 ? 'text-green-600' : 'text-red-600'}`}>
          ...
          <div className="text-center py-12">
            ...
            <h3 className="text-lg font-medium text-gray-900 mb-1">Aucune monture trouvée</h3>
            <p className="text-gray-600 mb-6">Vous n'avez pas encore créé de monture</p>
            <button
              onClick={() => navigate('/montures/create')}
