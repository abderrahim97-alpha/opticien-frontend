...
const OpticienDetails: React.FC = () => {
  ...
  useEffect(() => {
    if (selectedImageIndex === null || !opticien?.images) return;
    ...
    const handleKeyDown = (e: KeyboardEvent) => {
      if (e.key === 'Escape') {
        ...
        setSelectedImageIndex(prev => {
          if (prev === null || !opticien?.images) return prev;
          return prev === 0 ? opticien.images.length - 1 : prev - 1;
        });
      } else if (e.key === 'ArrowRight') {
        e.preventDefault();
        setSelectedImageIndex(prev => {
          if (prev === null || !opticien?.images) return prev;
          return prev === opticien.images.length - 1 ? 0 : prev + 1;
        });
      }
    };
  ...
  }, [selectedImageIndex, opticien?.images]);
  ...
  const LightboxModal = () => {
    if (selectedImageIndex === null || !opticien?.images) return null;
